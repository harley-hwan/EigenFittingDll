...
FILE* fp_log1 = nullptr;
FILE* fp_log2 = nullptr;
FILE* fp_log3 = nullptr;

//코스 별 조건 저장 파일_지면 높낮이와 바람 세기 정보
FILE* fp_mapinfo = nullptr;//타깃 지점 지면 높이 정보 저장할 문서
FILE* fp_windinfo = nullptr;//바람 영향 정보

CMyFilter::CMyFilter()
{
	fp_log1 = fopen("functionlog.txt", "a+");
	if (fp_log1 != nullptr)
	{
		fprintf(fp_log1, "\n\nVersion: 210913-2");
		fclose(fp_log1);
...
void CMyFilter::InitFitting()
{
	...
	auto its_time_t = std::chrono::system_clock::to_time_t(std::chrono::system_clock::now());
	...
	//FILE* fp = nullptr;
	// fp = fopen("settingdata.txt", "r");  //test파일을 r(읽기) 모드로 열기
	//double a, b;
	//int i = 0;
	//if (fp != nullptr)
	//{
	...
	fp_mapinfo = fopen("mapinfo.txt", "r");  //파일을 r(읽기) 모드로 열기
	if (nullptr != fp_mapinfo)
	{
		fseek(fp_mapinfo, 0L, SEEK_SET);
		fscanf(fp_mapinfo, "%f", &fgrounddata);
	...
	m_WindEffect = 1.0;
	float fwinddata = 1.0;
	fp_windinfo = fopen("windeffect.txt", "r");  //파일을 r(읽기) 모드로 열기
	if (nullptr != fp_windinfo)
	{
		fseek(fp_windinfo, 0L, SEEK_SET);
		fscanf(fp_windinfo, "%f", &fwinddata);
...
double CMyFilter::LowPassFilter_Process(double Input, double pastInput, double pastOutput)
{
	std::cout << __PRETTY_FUNCTION__ << " -> "
		<< LOG_ARG_("Input", Input)
		<< LOG_ARG_("pastInput", pastInput)
		<< LOG_ARG("pastOutput", pastOutput)
		<< std::endl;
...
double CMyFilter::BallDataFilter_Process(double Input, double degree, double pastInput)
{
	std::cout << __PRETTY_FUNCTION__ << " -> "
		<< LOG_ARG_("Input", Input)
		<< LOG_ARG_("degree", degree)
		<< LOG_ARG("pastInput", pastInput)
		<< std::endl;
...
{
	std::cout << __PRETTY_FUNCTION__ << " -> "
		<< LOG_ARG_("end_data", end_data)
		<< LOG_ARG_("ballspeed", ballspeed)
		<< LOG_ARG_("balldir_v", balldir_v)
		<< LOG_ARG("m_balldir_h", m_balldir_h)
		<< std::endl;
	//입력 좌표 로그 추가
	...
	if (end_data)
	{
		fp_log3 = fopen("datalog_input.txt", "w");//실시간 입력 좌표 기록
		if (fp_log3 != nullptr)
		{
...
////2021-08-02-좌우 굴곡 줄이기 테스트_start
			...
			if (fp_log1 != nullptr)
			{
				fprintf(fp_log1, "\n%2d-%2d-%2d-%2d-%2d: polyfit()", its_time->tm_mon, its_time->tm_mday + 1, its_time->tm_hour, its_time->tm_min, its_time->tm_sec);
				fprintf(fp_log1, "\n%2d-%2d-%2d-%2d-%2d: polyfit condition - end data: %1d, Input.y: %3.1f, ballspeed: %3.1f, dir_h: %2.1f, dir_v: %2.1f, limitdist: %3.1f"
					, its_time->tm_mon, its_time->tm_mday + 1, its_time->tm_hour, its_time->tm_min, its_time->tm_sec, end_data, Input.y, ballspeed, m_balldir_h, m_balldir_v, m_LimitDistance_forPloyfit);
				fclose(fp_log1);
				fp_log1 = nullptr;
			}
		}
		if (end_data)//나중에 end_data가 짧으면 폴리피팅 진행하지 않도록 수정해야 함
		{
			...
			if (fp_log1 != nullptr)
			{
				fprintf(fp_log1, "\n%2d-%2d-%2d-%2d-%2d: data end - Index status", its_time->tm_mon, its_time->tm_mday + 1, its_time->tm_hour, its_time->tm_min, its_time->tm_sec);
				fprintf(fp_log1, "\nlowpass_index: %3d, input_index: %3d, total_index: %3d", lowpass_endindex, end_index, input_index);
				fclose(fp_log1);
				fp_log1 = nullptr;
			}

			fp_log2 = fopen("datalog.txt", "w");//fitting 결과 데이터 기록
			if (fp_log2 != nullptr)
			{
				for(int i = 0; i<input_index; i++)
...
{
	std::cout << __PRETTY_FUNCTION__ << " -> "
		<< LOG_ARG_("input_y", input_y)
		<< LOG_ARG("input_y", diff_y)
		<< std::endl;
