...
extern "C" {
	...
	void InitFitting();//변수 초기화, 샷 레디 시마다 호출하기
	int Filter_Result(double Input_x, double Input_y, double Input_z, bool end_data, double ballspeed, double balldir_h, double balldir_v);//x:좌우, y: 거리, z: 높이 실시간 입력 좌표, end_data: 마지막 데이터이면 true, return 값이 1이면 fitting 완료, end_data true일 경우 return값도 1이 됨
