...
int Filter_Result(double Input_x, double Input_y, double Input_z, bool end_data, double ballspeed, double balldir_h, double balldir_v)//x:좌우, y: 거리, z: 높이
{
	std::cout << __PRETTY_FUNCTION__ << " -> "
		<< LOG_ARG_("Input_x", Input_x)
		<< LOG_ARG_("Input_y", Input_y)
		<< LOG_ARG_("Input_z", Input_z)
		<< LOG_ARG_("end_data", end_data)
		<< LOG_ARG_("ballspeed", ballspeed)
		<< LOG_ARG_("balldir_h", balldir_h)
		<< LOG_ARG("balldir_v", balldir_v)
		<< std::endl;
...
_FilterResult GetResultData()// Filter_Result() 함수 리턴값이 1이면 호출 중단하고, 한꺼번에 처리된 데이터 입력을 위해 total_index까지 데이터 읽어서 처리함
{
	std::cout << __PRETTY_FUNCTION__ << std::endl;
	return result_data;
}